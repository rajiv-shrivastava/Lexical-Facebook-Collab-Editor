...
import { $deleteWholeColumnAcrossGroup, $insertWholeColumnAcrossGroup } from '../../nodes/CustomTableNode/src/LexicalTableUtils';
...
function TableActionMenu({
  ...
  const insertTableColumnAtSelection = useCallback(
    (shouldInsertAfter: boolean) => {
      editor.update(() => {
        $insertWholeColumnAcrossGroup(editor, shouldInsertAfter, selectionCounts.columns);
        onClose();
      });
    },
  ...
  const deleteTableAtSelection = useCallback(() => {
    editor.update(() => {
      const tableNode = $getTableNodeFromLexicalNodeOrThrow(tableCellNode);
      const targetGroupId = tableNode.__groupId;

      if (!targetGroupId) {
        console.warn('No groupId found for selected table node');
        return;
      }
      ...
      // Recursive traversal to collect all table nodes with matching groupId
      const collectMatchingTables = (node: LexicalNode) => {
        if ($isTableNode(node) && node.__groupId === targetGroupId) {
          if(node.__isCaption === true){
            const para = editor.getElementByKey(node.getLatest().__key)
            if(para?.previousElementSibling && para.previousElementSibling !== null){
          const paraNode = $getNearestNodeFromDOMNode(para?.previousElementSibling)
          nodesToRemove.push(paraNode && paraNode !== null ? paraNode : node)
            }
          }
        ...
        if ($isElementNode(node)) {
          ...
          for (const child of children) {
            collectMatchingTables(child);
          }
        }
      };

      collectMatchingTables(root);
  ...
  const deleteTableColumnAtSelection = useCallback(() => {
    editor.update(() => {
      $deleteWholeColumnAcrossGroup(editor);
      onClose();
    });
  }, [editor, onClose]);
