...
import {
  ...
  MarkNode,
} from '../../nodes/lexical-mark-custom/index';
...
import addIcon from '../../../lexical/images/icons/addCommentIcon.svg';
import CheckIcon from "../../../lexical/images/icons/check-mark-line-icon.svg";
export const INSERT_INLINE_COMMAND: LexicalCommand<void> = createCommand(
  'INSERT_INLINE_COMMAND',
);
import closeIcon from "../../../lexical/images/icons/crossIcon.svg";
...
function PlainTextEditor({
  ...
  return (
    <LexicalComposer initialConfig={initialConfig}>
      ...
      <NewMentionsPlugin mentionItems={mentionItems} setmentionedEmails={setmentionedEmails}/>
    </LexicalComposer>
  );
}
...
function CommentInputBox({
    ...
    editor.getEditorState().read(() => {
      ...
      if ($isRangeSelection(selection)) {
        ...
        if (range !== null && boxElem !== null) {
          ...
          // console.log('setShowCommentInput 123123', boxElem.style.top)
          // if (parseFloat(boxElem.style.top) < 150) {
          //   console.log('can hide the box 123123')
  ...
  useEffect(() => {
    ...
    window.addEventListener('scroll', updateLocation, true); // capture scroll on any element
  ...
  const submitComment = () => {
    console.log("this is called" , wpAndTaskId)
    if (canSubmit) {
      ...
      submitAddComment(
        createThread(
          quote, 
          [createComment(content, (userName ?? author), userEmail, mentionedEmails)], 
          undefined,  
          wpAndTaskId
        ),
        true,
...
function CommentsComposer({
  ...
  const submitComment = () => {
    if (canSubmit) {
      setmentionedEmails([])
      submitAddComment(createComment(content, (userName ?? author) , userEmail , mentionedEmails), false, thread);
...
function CommentsPanelListComment({
  ...
  // const seconds = Math.round((Number(comment.timeStamp) - performance.now()) / 1000);
  ...
  return (
    <li className="CommentPlugin_CommentsPanel_List_Comment">
      <div className="CommentPlugin_CommentsPanel_List_Details">
        ...
        <span className="CommentPlugin_CommentsPanel_List_Comment_Time" style={{fontSize : "13px"}}>
    ...
    .map((part, i) => {
      ...
      // strip the '@' and lower-case
      const name = part.slice(1).toLowerCase();
      // check if any mentioned email matches this name
      const isMentioned = (comment.mentionedEmails ?? []).some(email =>
        email.split('@')[0].toLowerCase() === name
      );
      if (isMentioned) {
        return (
          <span key={i} className="CommentPlugin_Mention">
            {part}
          </span>
        );
      }
...
function CommentsPanelList({
  ...
  console.log('comments', comments , wpAndTaskId)
  const filteredComments = comments.filter((commentOrThread : any) =>
    commentOrThread.wpAndTaskId &&
    commentOrThread.wpAndTaskId.workplanId === wpAndTaskId.workplanId &&
    commentOrThread.wpAndTaskId.taskId === wpAndTaskId.taskId
  );

  console.log('Filtered comments', filteredComments, wpAndTaskId);
  return (
    <ul className="CommentPlugin_CommentsPanel_List" ref={listRef}>
      {filteredComments.length > 0 ? filteredComments.map((commentOrThread) => {
...
function CommentsPanel({
  ...
  const handleGoToComment = (id: string) => {
    ...
    setActiveIDs?.([id]); // <-- make sure you pass setActiveIDs from parent
  };
  useEffect(() => {
    const id = 'xpoct'
    if (!listRef.current) return;
    ...
    setTimeout(() => {
      setActiveIDs?.([commentId]);
    },7000);
  },[]);

  return (
    <div className="CommentPlugin_CommentsPanel">
      <div className='CommentPlugin_CommentsPanel_HeaderDiv'>
...
export default function CommentPlugin({
  ...
  const commentStore = useMemo(() => {
    if (!(editor as any)?._commentStore) {
      (editor as any)._commentStore = new CommentStore(editor);
    }
    return (editor as any)?._commentStore;
  }, [editor]);
  ...
  const deleteCommentOrThread = useCallback(
    (comment: Comment | Thread, thread?: Thread) => {
      ...
      if (comment.type === "comment") {
        if (userName === comment.author) {
          ...
          if (thread) {
            const allDeleted = thread.comments.every(
              (c) => c.id === comment.id || c.deleted === true
            );
  
            if (allDeleted) {
              commentStore.deleteCommentOrThread(thread);
  
              const markNodeKeys = markNodeMap.get(thread.id);
              if (markNodeKeys) {
                setTimeout(() => {
                  editor.update(() => {
                  ...
                  handleSaveComments(commentStore.getComments());
                });
              } else {
                handleSaveComments(commentStore.getComments());
              }
        ...
        if (comment.comments?.[0].author == userName) {
    ...
    [commentStore, editor, markNodeMap, handleSaveComments, userName]
  );
  ...
  const submitAddComment = useCallback(
    (
      ...
      console.log('submitAddComment is called')
  ...
  const resolveThread = (thread?: Thread) => {
