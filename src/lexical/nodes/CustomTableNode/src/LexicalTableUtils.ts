...
import type {ElementNode, LexicalEditor, PointType} from 'lexical';
...
function $applyColumnInsertToGrid(
  grid: any,                // TableNode
  insertAfterColumn: number // -1 means "insert at start"
): { firstInsertedCell: TableCellNode | null } {
  const gridFirstChild = grid.getFirstChild(); // <tr>
  if (!gridFirstChild || !$isTableRowNode(gridFirstChild)) {
    return { firstInsertedCell: null };
  }

  // Build map for THIS grid
  const firstCell : any = gridFirstChild.getFirstChild(); // <td>/<th>
  if (!firstCell) return { firstInsertedCell: null };

  const [gridMap] = $computeTableMap(grid, firstCell, firstCell);
  if (!gridMap || !gridMap[0]) return { firstInsertedCell: null };

  const rowCount = gridMap.length;

  let firstInsertedCell: TableCellNode | null = null;
  const $createTableCellNodeForInsertTableColumn = (
    headerState: TableCellHeaderState = TableCellHeaderStates.NO_STATUS,
  ) => {
    const cell = $createTableCellNode(headerState).append($createParagraphNode());
    if (firstInsertedCell === null) firstInsertedCell = cell;
    return cell;
  };
  ...
  rowLoop: for (let i = 0; i < rowCount; i++) {
    if (i !== 0) {
      ...
      if (!$isTableRowNode(currentRow)) break;
      loopRow = currentRow;
    }

    const rowMap = gridMap[i];

    const headerProbeIndex = insertAfterColumn < 0 ? 0 : Math.min(insertAfterColumn, rowMap.length - 1);
    const currentCellHeaderState = (rowMap[headerProbeIndex].cell as TableCellNode).__headerState;
    ...
    // Normal path (mirror of core insertColumn for this grid)
    const {
      cell: currentCell,
    ...
    if (currentStartColumn + currentCell.__colSpan - 1 <= insertAfterColumn) {
      ...
      // We’re inside a spanning cell → increase its colspan
...
export function $insertWholeColumnAcrossGroup(
  ...
  // Derive boundary from ACTIVE grid
  ...
  let insertAfterColumn = insertAfter
    ? startColumn + focusCell.__colSpan - 1
    : startColumn - 1;

  // Find the table group
  const gridEl = editor.getElementByKey(grid.getKey());
  const tableGroupId = gridEl?.getAttribute('data-table-id');

  // Fallback: operate only on active grid
  const collectTargets = (): any[] => {
    if (!tableGroupId) return [grid];
    const targets: any[] = [];
    const root = $getRoot();
    root.getChildren().forEach((node) => {
      if ($isTableNode(node)) {
        const el = editor.getElementByKey(node.getKey());
        if (el?.getAttribute('data-table-id') === tableGroupId) {
          targets.push(node);
        }
      }
    });
  ...
  // Perform N insertions (to match your selectionCounts loop)
  let firstInsertedInActive: TableCellNode | null = null;

  for (let k = 0; k < repeat; k++) {
    targets.forEach((t) => {
      const { firstInsertedCell } = $applyColumnInsertToGrid(t, insertAfterColumn);
      // Remember selection movement for the active grid only
      if (t.getKey() === grid.getKey() && firstInsertedInActive === null) {
        firstInsertedInActive = firstInsertedCell;
      }
    });

    // If inserting AFTER, shift the boundary right for subsequent repetitions
    if (insertAfter) insertAfterColumn += 1;
  }

  // Move caret like core API does (use the first inserted cell in the active grid)
  if (firstInsertedInActive) {
    $moveSelectionToCell(firstInsertedInActive);
  } else if (gridMap?.[0]?.[0]) {
    // Safe fallback: keep selection on the active grid
    const { cell } = gridMap[0][Math.max(0, Math.min(insertAfterColumn, gridMap[0].length - 1))];
    if (cell) $moveSelectionToCell(cell);
  }
}
...
function $applyColumnDeleteToGrid(
  grid: any, // TableNode
  startColumn: number,
  endColumn: number
): { columnCountBefore: number } {
  const rowNode = grid.getFirstChild(); // <tr>
  if (!rowNode) return { columnCountBefore: 0 };

  const firstCell = rowNode.getFirstChild(); // <td>/<th>
  if (!firstCell /* || !$isTableCellNode(firstCell) */) {
    return { columnCountBefore: 0 };
  }

  // ✅ Build the map correctly
  const [gridMap] = $computeTableMap(grid, firstCell, firstCell);
  if (!gridMap || gridMap.length === 0 || !gridMap[0]) {
    return { columnCountBefore: 0 };
  }

  const rowCount = gridMap.length;         // should be 1 for your 1-row tables
  const columnCount = gridMap[0].length;

  // If selection is completely outside this row, no-op
  if (startColumn > columnCount - 1) {
    return { columnCountBefore: columnCount };
  }

  // Clamp to this row’s bounds so we don’t index undefined
  const clampedStart = Math.max(0, startColumn);
  const clampedEnd = Math.min(endColumn, columnCount - 1);
  const selectedColumnCount = clampedEnd - clampedStart + 1;

  // If this row would become empty, remove the whole table row (your 1-row table)
  if (columnCount === selectedColumnCount) {
    grid.remove();
    return { columnCountBefore: columnCount };
  }

  // Delete/adjust cells in the selected range
  for (let row = 0; row < rowCount; row++) {
    for (let column = clampedStart; column <= clampedEnd; column++) {
      const entry = gridMap[row][column];
      if (!entry) continue;

      const { cell, startColumn: cellStartColumn } = entry;

      if (cellStartColumn < clampedStart) {
        // Cell overlaps from left into the deletion window
        if (column === clampedStart) {
          const overflowLeft = clampedStart - cellStartColumn;
          cell.setColSpan(
            cell.__colSpan -
              Math.min(selectedColumnCount, cell.__colSpan - overflowLeft)
          );
        }
      } else if (cellStartColumn + cell.__colSpan - 1 > clampedEnd) {
        // Cell overlaps from right into the deletion window
        if (column === clampedEnd) {
          const inSelectedArea = clampedEnd - cellStartColumn + 1;
          cell.setColSpan(cell.__colSpan - inSelectedArea);
        }
      } else {
        // Cell fully inside deletion window
        cell.remove();
      }
    }
  }
...
export function $deleteWholeColumnAcrossGroup(editor: LexicalEditor): void {
  const selection = $getSelection();
  invariant(
    $isRangeSelection(selection) || $isTableSelection(selection),
    'Expected a RangeSelection or TableSelection',
  ...
  const thisRowColumnCount = gridMap?.[0]?.length ?? 0;

  // ✅ Safely read the group id
  const gridEl = editor.getElementByKey(grid.getKey());
  const tableGroupId = gridEl?.getAttribute('data-table-id');
  if (!tableGroupId) {
    // Fallback: just delete on current table
    $applyColumnDeleteToGrid(grid, startColumn, endColumn);
    return;
  }

  // Collect all tables in the same group
  const allTablesInGroup: any[] = [];
  const root = $getRoot();
  root.getChildren().forEach((node) => {
    if ($isTableNode(node)) {
      const el = editor.getElementByKey(node.getKey());
      if (el?.getAttribute('data-table-id') === tableGroupId) {
        allTablesInGroup.push(node);
      }
    }
  });

  // If deleting all columns for this row, remove the whole group
  if (thisRowColumnCount > 0 && thisRowColumnCount === selectedColumnCount) {
    grid.selectPrevious();
    allTablesInGroup.forEach((t) => t.remove());
    return;
  }

  // Apply to every row (each 1-row table)
  allTablesInGroup.forEach((t) => {
    $applyColumnDeleteToGrid(t, startColumn, endColumn);
  });

  // Keep your original selection movement logic for the active grid
