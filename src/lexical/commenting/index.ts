...
// Example:
console.log(getCurrentDate()); // e.g. "Apr 25"
...
export function createComment(
  ...
  return {
    ...
    timeStamp: timeStamp === undefined ? `${getCurrentTime()} - ${getCurrentDate()}` : timeStamp,
    type: "comment",
    userEmail: userEmail === undefined ? "" : userEmail,
    mentionedEmails: mentionedEmails === undefined ? [] : mentionedEmails,
  };
}
...
export function createThread(
  ...
  console.log('wpAndTaskId 2222222222222', wpAndTaskId)
  return {
    comments,
    id: id === undefined ? createUID() : id,
    quote,
...
function cloneThread(thread: Thread): Thread {
  console.log('thread clone is called', thread)
  return {
    ...
    wpAndTaskId: thread.wpAndTaskId,
  };
}
...
function markDeleted(comment: Comment): Comment {
  return {
    ...
    mentionedEmails: comment.mentionedEmails,
  };
}
...
export class CommentStore {
  ...
  resolveThread(thread: Thread) {
    const resolvedThread = {
      ...thread,
      resolved: true, // Mark the thread as resolved
    };
      const allComments = this._comments.map((comment) => {
      if (comment.type === "thread" && comment.id === resolvedThread.id) {
        return resolvedThread; // Replace the old thread with the resolved one
      }
      return comment;
    });
  ...
  _createCollabSharedMap(commentOrThread: Comment | Thread): YMap<any> {
    ...
    if (type === "comment") {
      ...
      sharedMap.set("userEmail", commentOrThread.userEmail);
    } else {
                    ...
                    const commentOrThread =
                      type === "thread"
                        ? createThread(
                              ...
                              .map(
                                (
                                  ...
                                  createComment(
                                    ...
                                    innerComment.get("userEmail") as string,
                                    innerComment.get('mentionedEmails') as any,
                            ...
                            id,
                            map.get("wpAndTaskId") as wpAndTaskIdType,
                          )
                        : createComment(
                            map.get("content"),
                            map.get("author"),
                            map.get("userEmail"),
                            map.get('mentionedEmails'),
                            id,
                            map.get("timeStamp"),
                            map.get("deleted"),
                          );
